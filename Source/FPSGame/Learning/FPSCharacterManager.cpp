...
AFPSCharacterManager::AFPSCharacterManager()
{
	...
	if (FParse::Value(*CommandLine, TEXT("-LearningRatePolicy="), LearningRatePolicyStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: LearningRatePolicy set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-LearningRateCritic="), LearningRateCriticStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: LearningRateCritic set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-EpsilonClip="), EpsilonClipStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: EpsilonClip set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-PolicyBatchSize="), PolicyBatchSizeStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: PolicyBatchSize set from command line: %d"),
	...
	if (FParse::Value(*CommandLine, TEXT("-CriticBatchSize="), CriticBatchSizeStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: CriticBatchSize set from command line: %d"),
	...
	if (FParse::Value(*CommandLine, TEXT("-IterationsPerGather="), IterationsPerGatherStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: IterationsPerGather set from command line: %d"),
	...
	if (FParse::Value(*CommandLine, TEXT("-NumberOfIterations="), NumberOfIterationsStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: NumberOfIterations set from command line: %d"),
	...
	if (FParse::Value(*CommandLine, TEXT("-DiscountFactor="), DiscountFactorStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: DiscountFactor set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-GaeLambda="), GaeLambdaStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: GaeLambda set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-ActionEntropyWeight="), ActionEntropyWeightStr))
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: ActionEntropyWeight set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-UseObstacles="), UseObstaclesStr))
	{
		...
		// UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: UseObstacles set from command line: %s"),
	...
	if (FParse::Value(*CommandLine, TEXT("-MaxObstacles="), MaxObstaclesStr))
	{
		...
		// UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: MaxObstacles set from command line: %d"),
	...
	if (FParse::Value(*CommandLine, TEXT("-MinObstacleSize="), MinObstacleSizeStr))
	{
		...
		// UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: MinObstacleSize set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-MaxObstacleSize="), MaxObstacleSizeStr))
	{
		...
		// UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: MaxObstacleSize set from command line: %f"),
	...
	if (FParse::Value(*CommandLine, TEXT("-ObstacleMode="), ObstacleModeStr))
	{
		...
		// UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: ObstacleMode set from command line: %s"),
	...
	if (bIsHeadlessTraining)
	{
		...
		UE_LOG(LogTemp, Log,
		       TEXT("FPSCharacterManager: Headless training detected, forcing RunMode to ReInitialize: %d"),
		       (int32)RunMode);
	}
	else
	{
		...
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: Editor mode detected, using Blueprint RunMode: %d"),
		       (int32)RunMode);
	}
	...
	else
	{
		...
		if (!bFoundPath)
		{
			...
			UE_LOG(LogTemp, Warning,
			       TEXT("FPSCharacterManager: Unreal Engine not found in common locations, using default: %s"),
			       *EnginePath);
		}
	}
#elif PLATFORM_LINUX
...
void AFPSCharacterManager::InitializeAgents()
{
	...
	for (AActor* Actor : AllCharacters)
	{
		if (IsValid(Actor) && Actor->IsA<AFPSCharacter>())
		{
			...
			{
				Agents.Add(Actor);
				UE_LOG(LogTemp, Warning, TEXT("FPSCharacterManager: Added character %s (derived from FPSCharacter)"),
				       *Actor->GetName());
			}
		}
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("FPSCharacterManager: Learning Manager MaxAgentNum: %d"),
	       LearningAgentsManager->GetMaxAgentNum());
	...
	for (int32 i = 0; i < AllCharacters.Num(); i++)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Character %d: %s (Class: %s) - Is FPSCharacter: %s, In Agents List: %s"), i,
		       *Actor->GetName(), *Actor->GetClass()->GetName(), bIsFPSCharacter ? TEXT("YES") : TEXT("NO"),
	...
	for (int32 AgentIndex = 0; AgentIndex < Agents.Num(); AgentIndex++)
	{
		...
		if (APawn* Pawn = Cast<APawn>(Agent))
		{
			...
			if (!ExistingController)
			{
				...
				if (World)
				{
					...
					if (NewController)
					{
						...
						UE_LOG(LogTemp, Warning, TEXT("FPSCharacterManager: Created AIController for agent %s"),
						       *Agent->GetName());
					}
					else
					{
						UE_LOG(LogTemp, Error, TEXT("FPSCharacterManager: Failed to create AIController for agent %s"),
						       *Agent->GetName());
			...
			{
				UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: Agent %s already has controller %s"), *Agent->GetName(),
		...
		{
			UE_LOG(LogTemp, Error, TEXT("FPSCharacterManager: Failed to add agent %s to manager (returned INDEX_NONE)"),
			       *Agent->GetName());
			continue;
		}
		...
		if (AgentId != SuccessfulAgentIds.Num())
		{
			UE_LOG(LogTemp, Error,
			       TEXT("FPSCharacterManager: Agent %s got non-sequential ID %d (expected %d) - this may cause "
			            "multi-agent issues"),
			       *Agent->GetName(), AgentId, SuccessfulAgentIds.Num());
		}

		SuccessfulAgentIds.Add(AgentId);
		UE_LOG(LogTemp, Warning, TEXT("FPSCharacterManager: Successfully added agent %s to manager with ID %d"),
		...
		if (RunMode == EFPSCharacterManagerMode::Inference)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: Agent %s initialized in inference mode"),
			       *Agent->GetName());
		}
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("FPSCharacterManager: Successfully registered %d agents with learning manager"),
	       RegisteredAgentCount);
	UE_LOG(LogTemp, Warning, TEXT("FPSCharacterManager: Agent IDs are sequential: %s"),
	       bHasSequentialIds ? TEXT("YES") : TEXT("NO"));

	if (Agents.Num() == 0)
	{
		UE_LOG(LogTemp, Error, TEXT("FPSCharacterManager: No FPSCharacter agents found! Make sure to:"));
		UE_LOG(LogTemp, Error,
		       TEXT("1. Place FPSCharacter (or Blueprint derived from FPSCharacter) actors in your level"));
		UE_LOG(LogTemp, Error,
		       TEXT("2. Don't just set FPSCharacter as PlayerPawn - you need actual actors in the world"));
	...
	{
		UE_LOG(LogTemp, Error, TEXT("FPSCharacterManager: Only %d out of %d agents were successfully registered!"),
		       RegisteredAgentCount, Agents.Num());
	...
	{
		UE_LOG(LogTemp, Error,
		       TEXT("FPSCharacterManager: Only 1 agent registered despite finding %d agents - possible learning "
		            "manager limitation"),
		       Agents.Num());
	}
	...
	{
		UE_LOG(LogTemp, Error,
		       TEXT("FPSCharacterManager: Agent IDs are not sequential starting from 0 - this WILL cause multi-agent "
		            "issues!"));
...
void AFPSCharacterManager::InitializeManager()
{
	...
	{
		UE_LOG(LogTemp, Warning,
		       TEXT("FPSCharacterManager: Headless training detected, forcing RunMode from %d to ReInitialize"),
		       (int32)RunMode);
		RunMode = EFPSCharacterManagerMode::ReInitialize;
	}
	else if (!bIsHeadlessTraining)
	{
		UE_LOG(LogTemp, Log, TEXT("FPSCharacterManager: Editor mode - respecting Blueprint RunMode: %d"),
		       (int32)RunMode);
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("FPSCharacterManager: ReInitialize mode: %s"),
	       ReInitialize ? TEXT("YES") : TEXT("NO"));
	...
	Policy = ULearningAgentsPolicy::MakePolicy(ManagerPtr, InteractorPtr, ULearningAgentsPolicy::StaticClass(),
	                                           TEXT("FPSCharacter Policy"), EncoderNeuralNetwork, PolicyNeuralNetwork,
	                                           DecoderNeuralNetwork, ReInitialize, ReInitialize, ReInitialize,
	                                           ModifiedPolicySettings, RandomSeed);
	if (Policy == nullptr)
	{
	...
	Critic = ULearningAgentsCritic::MakeCritic(ManagerPtr, InteractorPtr, PolicyPtr,
	                                           ULearningAgentsCritic::StaticClass(), TEXT("FPSCharacter Critic"),
	...
	TrainingEnvironment =
	    Cast<UFPSCharacterTrainingEnvironment>(ULearningAgentsTrainingEnvironment::MakeTrainingEnvironment(
	...
	PPOTrainer = ULearningAgentsPPOTrainer::MakePPOTrainer(
	    ...
	    ULearningAgentsPPOTrainer::StaticClass(), TEXT("FPSCharacter PPO Trainer"), ModifiedTrainerSettings);
...
void AFPSCharacterManager::Tick(float DeltaTime)
{
	...
	if (DebugTimer >= 2.0f) // Log every 2 seconds
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Run Mode: %s"),
		       ...
		       : RunMode == EFPSCharacterManagerMode::Inference ? TEXT("Inference")
		                                                        : TEXT("ReInitialize"));

		for (int32 AgentId : AgentIds)
		{
			AFPSCharacter* Character =
			    Cast<AFPSCharacter>(LearningAgentsManager->GetAgent(AgentId, AFPSCharacter::StaticClass()));
			if (Character)
			{
				FVector Velocity = Character->GetCharacterMovement() ? Character->GetCharacterMovement()->Velocity
				                                                     : FVector::ZeroVector;
				UE_LOG(LogTemp, Warning, TEXT("Agent %d (%s): Location=(%s), Velocity=(%s), Moving=%s"), AgentId,
				       *Character->GetName(), *Character->GetActorLocation().ToString(), *Velocity.ToString(),
