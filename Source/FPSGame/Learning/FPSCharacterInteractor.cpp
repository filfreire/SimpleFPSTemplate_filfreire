	...
	if (!Character || !TargetActor)
	{
		...
		if (!TargetActor)
		{
			UE_LOG(
			    LogTemp, Error,
			    TEXT(
			        "FPSCharacterInteractor: TargetActor is NULL - make sure FPSCharacterManager.TargetActor is set!"));
		}
		return;
	}
    ...
    FLearningAgentsActionSchemaElement& OutActionSchemaElement, ULearningAgentsActionSchema* InActionSchema)
{
	...
	CharacterActions.Add("LookUp", ULearningAgentsActions::SpecifyFloatAction(InActionSchema, 1.0f, "FloatAction"));
    ...
    const int32 AgentId)
{
	...
	if (!Character)
	{
		UE_LOG(LogTemp, Error,
		       TEXT("FPSCharacterInteractor: Failed to get character for agent %d in PerformAgentAction"), AgentId);
		return;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("AGENT %d ACTION: Forward=%.3f, Right=%.3f, Turn=%.3f, LookUp=%.3f"), AgentId,
		       MoveForwardValue, MoveRightValue, TurnValue, LookUpValue);
	}
