...
void UFPSCharacterTrainingEnvironment::GatherAgentCompletion_Implementation(ELearningAgentsCompletion& OutCompletion,
                                                                            const int32 AgentId)
{
...
void UFPSCharacterTrainingEnvironment::ResetAgentEpisode_Implementation(const int32 AgentId)
{
	...
	if (!Character || !TargetActor)
	{
		UE_LOG(LogTemp, Error,
		       TEXT("FPSCharacterTrainingEnvironment: Reset failed for Agent %d - Character: %s, Target: %s"), AgentId,
		       Character ? TEXT("Valid") : TEXT("NULL"), TargetActor ? TEXT("Valid") : TEXT("NULL"));
		return;
	}
	...
	FVector TraceStart =
	    FVector(CharacterResetLocation.X, CharacterResetLocation.Y, ResetCenter.Z + ResetBounds.Z + 1000.0f);
	FVector TraceEnd =
	    FVector(CharacterResetLocation.X, CharacterResetLocation.Y, ResetCenter.Z - ResetBounds.Z - 1000.0f);
	...
	if (AgentId == 0 || !TargetActor)
	{
		...
		do
		{
			...
			FVector TargetTraceStart =
			    FVector(TargetResetLocation.X, TargetResetLocation.Y, ResetCenter.Z + ResetBounds.Z + 1000.0f);
			FVector TargetTraceEnd =
			    FVector(TargetResetLocation.X, TargetResetLocation.Y, ResetCenter.Z - ResetBounds.Z - 1000.0f);
			...
			if (Character->GetWorld()->LineTraceSingleByChannel(TargetHitResult, TargetTraceStart, TargetTraceEnd,
			                                                    ECC_WorldStatic, TargetQueryParams))
			{
...
void UFPSCharacterTrainingEnvironment::ConfigureObstacles(bool bUse, int32 MaxObs, float MinSize, float MaxSize,
                                                          EObstacleMode Mode)
{
	bUseObstacles = bUse;
	MaxObstacles = MaxObs;
	...
	// UE_LOG(LogTemp, Log, TEXT("FPSCharacterTrainingEnvironment: Obstacles configured - Use: %s, Max: %d, MinSize: %f,
